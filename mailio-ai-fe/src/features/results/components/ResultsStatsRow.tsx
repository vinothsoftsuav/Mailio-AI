import {
  Mail, ShieldCheck, XCircle, AlertTriangle,
  TrendingUp, TrendingDown, Minus,
} from "lucide-react";
import { cn } from "@/src/lib/utils";
import { MOCK_RESULTS_STATS } from "../mock";
...
export function ResultsStatsRow() {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {MOCK_RESULTS_STATS.map((stat) => {
        const Icon  = ICON_MAP[stat.iconName] ?? Mail;
        const isUp  = stat.change > 0;
        const isDown = stat.change < 0;

        return (
          <div key={stat.id} className="flex items-start gap-3 rounded-xl border border-border bg-card p-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className={cn(
                  "text-[11px]",
                  stat.change === 0 ? "text-muted-foreground" : isUp ? "text-emerald-600" : "text-red-500"
                )}>
                  {stat.subLabel}
                </span>
              </div>
